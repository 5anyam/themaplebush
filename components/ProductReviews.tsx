...
const ProductReviews: React.FC<ProductReviewsProps> = ({ productId, productName }) => {
  ...
  const API_BASE = 'https://cms.thecurioshelf.com/wp-json/wc/v3';
    ...
    return (
      <div className="flex gap-0.5">
        {[1, 2, 3, 4, 5].map((star) => (
          ...
          >
            {star <= rating ? (
              <StarIcon className={`${sizeClass} text-[#E11D74]`} />
            ) : (
              <StarOutlineIcon className={`${sizeClass} text-gray-300 ${interactive ? 'hover:text-[#E11D74]/50' : ''} transition-colors`} />
            )}
          </button>
        ))}
      </div>
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
          ...
          <div>
            <div className="flex items-center gap-2 mb-1">
              <MessageSquare className="w-5 h-5 text-[#E11D74]" />
              ...
              {[5, 4, 3, 2, 1].map((star) => {
                ...
                return (
                  <div key={star} className="flex items-center gap-2">
                    ...
                    <StarIcon className="w-3 h-3 text-[#E11D74] flex-shrink-0" />
                    <div className="flex-1 h-1.5 bg-gray-100 rounded-full overflow-hidden">
                      <div
                        className="h-full bg-[#E11D74] rounded-full transition-all duration-500"
                        style={{ width: `${pct}%` }}
                      />
                    </div>
          ...
          <button
            ...
            className={`inline-flex items-center gap-2 px-7 py-3 rounded-xl font-semibold text-sm transition-all duration-300 ${
              showForm
                ...
                : 'bg-[#E11D74] text-white hover:bg-[#B01361] shadow-md hover:shadow-lg hover:shadow-orange-200 hover:-translate-y-0.5 active:translate-y-0'
            }`}
          >
            {showForm ? (
          ...
          >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-semibold text-gray-600 mb-1.5 uppercase tracking-wider">
                  Name <span className="text-[#E11D74]">*</span>
                </label>
                <input
                  type="text"
                  required
                  ...
                  className="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-[#E11D74] focus:outline-none focus:ring-2 focus:ring-[#E11D74]/10 text-sm transition-all"
            ...
            <div>
              <label className="block text-xs font-semibold text-gray-600 mb-2 uppercase tracking-wider">
                Rating <span className="text-[#E11D74]">*</span>
              </label>
            ...
            <div>
              <label className="block text-xs font-semibold text-gray-600 mb-1.5 uppercase tracking-wider">
                Your Review <span className="text-[#E11D74]">*</span>
              </label>
              <textarea
                required
                ...
                rows={4}
                className="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-[#E11D74] focus:outline-none focus:ring-2 focus:ring-[#E11D74]/10 text-sm resize-none transition-all"
            ...
            <button
              ...
              className={`w-full py-3.5 rounded-xl text-sm font-bold uppercase tracking-wide transition-all duration-300 flex items-center justify-center gap-2 ${
                submitting
                  ...
                  : 'bg-[#E11D74] text-white hover:bg-[#B01361] shadow-md hover:shadow-lg hover:shadow-orange-200'
              }`}
            >
              {submitting ? (
        ...
        {loading ? (
          <div className="py-16 text-center">
            <div className="relative w-12 h-12 mx-auto mb-4">
              ...
              <ShoppingBag className="absolute inset-0 m-auto w-5 h-5 text-[#E11D74]" />
            </div>
            <p className="text-gray-500 text-sm">Loading reviews...</p>
          </div>
        ) : reviews.length === 0 ? (
          <div className="py-16 text-center border-2 border-dashed border-gray-100 rounded-2xl">
            <div className="w-16 h-16 bg-[#FFE9DD] rounded-full flex items-center justify-center mx-auto mb-4">
              <MessageSquare className="w-8 h-8 text-[#E11D74]/40" />
            </div>
            <p className="text-gray-900 font-semibold mb-1">No Reviews Yet</p>
            <p className="text-gray-500 text-sm">
              Be the first to review{' '}
              <span className="text-[#E11D74] font-medium">{productName}</span>
            </p>
          </div>
        ) : (
          <div className="space-y-4">
            {reviews.map((r) => (
              <div
                key={r.id}
                className="p-5 md:p-6 bg-white border border-gray-100 rounded-2xl hover:border-[#E11D74]/20 hover:shadow-sm transition-all duration-300"
              >
                {/* Reviewer info */}
                <div className="flex items-start gap-3 mb-4">
                  <div className="w-10 h-10 rounded-full bg-gradient-to-br from-[#E11D74] to-[#ff8c00] flex items-center justify-center flex-shrink-0 shadow-md shadow-orange-200">
                  ...
                  <div className="mt-4 flex gap-2 flex-wrap ml-13">
                    {r.images.map((src, i) => (
                      <button
                        ...
                        className="relative group/img overflow-hidden rounded-xl border-2 border-gray-100 hover:border-[#E11D74]/40 transition-all"
                      >
                        <img
                          src={src}
